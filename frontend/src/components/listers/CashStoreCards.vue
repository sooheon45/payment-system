    ...
    <div>
        ...
        >
            <v-row>
                <v-list-item class="d-flex" style="background-color: white;">
                    <h1 class="align-self-center ml-3">Store</h1>
        ...
        <v-col style="margin-bottom:40px;">
            <div class="text-center">
                ...
                >
                    <CashStore :offline="offline" class="video-card" :isNew="true" :editMode="true" v-model="newValue" 
        ...
        <v-row>
            <CashStore :offline="offline" class="video-card" v-for="(value, index) in values" v-model="values[index]" v-bind:key="index" @delete="remove"/>
        </v-row>
    </div>
</template>
...
<script>
    ...
    import CashStore from './../CashStore.vue';

    export default {
        name: 'CashStoreManager',
        components: {
            CashStore,
        },
        props: {
